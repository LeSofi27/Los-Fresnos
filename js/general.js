...
const iniciarCalendario = function(elemento){
    new AirDatepicker(elemento, {
        ...
        onSelect(valor){
            ...
            const estadia = diferenciaDeFechasEnDias(valor.date[0], valor.date[1]);
...
const actualizarCarrito = function(){
    ...
    document.querySelector('.carrito-footer').classList.remove('d-none');
    ...
    reservas.forEach(function(reserva, i){
        ...
        item.querySelector('.precio').innerHTML = "$" + Number(reserva.precio) + " por noche";
        item.querySelector('.estadia').innerHTML = "Noches seleccionadas: " + reserva.estadia;
    ...
    document.querySelector('.carrito-total').innerHTML = "$" + reservas.reduce(function(total, reserva, i){
...
(function(){
    document.body.style.paddingTop = document.body.querySelector('header').scrollHeight + "px";
